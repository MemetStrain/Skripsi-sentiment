"""
Multi-Horizon CPO Price Forecasting — C4 (Full Model: Price + HMM + Sentiment)
==============================================================================

Ablation configuration C4: lagged price + HMM market state + news sentiment.
...
Usage:
    python horizon_forecast_C4_full.py --interval daily
"""
...
# Per-source lag config (C4: full — price + HMM + sentiment).
...
# HMM_Log_Return / HMM_RSI / HMM_MACD removed per feature analysis (low gain).
# HMM_Close excluded — duplicate price anchor that confuses log-return target.
EXCLUDE_COLS = {
    'Date', 'Target',
    'Close_Anchor', 'Close', 'Log_Return',
    'Dominant_Sentiment',
    'HMM_Close', 'HMM_Log_Return', 'HMM_RSI', 'HMM_MACD',
}
...
def load_and_merge_data(interval: str) -> pd.DataFrame:
    """Load CPO + sentiment + HMM data, dropping raw same-day OHLCV columns."""
...
def engineer_features_for_horizon(df: pd.DataFrame, horizon: int
    ...
    """Build features for a specific forecast horizon (no look-ahead)."""
    df = df.copy()
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    print(f"\n{'='*60}")
    print(f"  {interval} - Horizon {horizon} (C4: full — price + HMM + sentiment)")
    print(f"{'='*60}")
    ...
    if len(data['dates_test']):
        ...
        ax.set_title(f'{interval} Forecast (Testing 2026+) - Horizon {horizon} (C4: full)',
    ...
    if test_results:
        ...
        fig.suptitle(f'{interval} Horizon {horizon} (Testing, C4) - BASE vs CSA',
                     fontsize=14, fontweight='bold')
        fig.tight_layout()
...
def _horizon_summary_plots(summary_df: pd.DataFrame, interval: str,
    ...
    for metric in ['RMSE', 'MAPE']:
        ...
        ax.set_title(f'{interval} - {metric} Across Horizons (C4, {tag.title()})',
                     fontsize=14, fontweight='bold')
    ...
    for (model, opt), grp in summary_df.groupby(['Model', 'Optimization']):
        ax.plot(grp['Horizon'], grp['R2_Price'], marker='o',
                linestyle='--' if opt == 'BASE' else '-',
                label=f'{model} ({opt})', linewidth=1.5)
    ax.set_title(f'{interval} - R² (Price Space) Across Horizons (C4, {tag.title()})',
                 fontsize=14, fontweight='bold')
...
def main():
    parser = argparse.ArgumentParser(
        description='Multi-Horizon CPO Price Forecasting — C4 (full ablation)')
