"""
Multi-Horizon CPO Price Forecasting with CSA and Bayesian Optimization
=======================================================================
...
Usage:
    ...
    python horizon_forecast.py --interval daily --optimizer bayesian
    python horizon_forecast.py --interval daily --optimizer both
"""
...
def _save_split_outputs(split_name: str, dates, y_true, close_anchor,
    ...
    """Save predictions CSV and results CSV for one data split (training/testing/validation)."""
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    Returns (testing_results_df, validation_results_df).
    """
    cfg = INTERVAL_CONFIGS[interval]
    ...
    # Feature engineering on the full dataset so lag values at the 2026 boundary
    # are computed from pre-2026 data (no NaN at the start of the val window).
    ...
    # Pre-2026 combined arrays used for re-fitting when producing validation predictions
    ...
    for model_type in model_types:
        ...
        if model_type in ('xgboost', 'random_forest'):
            ...
            # Validation: re-fit on all pre-2026 with same default params
            ...
            # Validation: re-fit on all pre-2026
            fitted_val = train_statsmodels(model_type, y_all_pre, exog_all_pre,
                                           order, seasonal_order)
        ...
        if bayes_config and bayes_config.get('enabled', False):
            ...
            bayes_opt = BayesianTimeSeriesOptimizer(
                ...
                config=bcfg,
                cv_folds=bayes_config['cv_folds'],
                n_calls=bayes_config['n_calls'],
                n_initial_points=bayes_config['n_initial_points'],
            )
            ...
            val_results[key]   = (calculate_metrics(data['y_val'], y_pred_val, data['close_val'])
                                  if len(data['y_val']) else {})
            all_params[key] = {**bayes_params,
                               'bayes_best_score': float(bayes_result.best_score),
                               'bayes_iterations': bayes_result.total_iterations}
            m = test_results[key]
    ...
    ax.set_title(f'{interval} Forecast (Testing) - Horizon {horizon}', fontsize=14, fontweight='bold')
    ...
    if len(data['val_dates']):
        ...
        for name, preds in val_preds.items():
            ...
            ax.plot(data['val_dates'], data['close_val'] * np.exp(safe),
                    label=name.replace('_', ' ').title(),
                    color=colors.get(name, '#999'), linewidth=1.1, linestyle=ls, alpha=0.8)
        ax.set_title(f'{interval} Forecast (Validation 2026) - Horizon {horizon}',
    ...
    fig.suptitle(f'{interval} Horizon {horizon} (Testing) - {opt_names}', fontsize=14, fontweight='bold')
    fig.tight_layout()
...
def _horizon_summary_plots(summary_df: pd.DataFrame, interval: str,
    ...
    """Write summary CSV and cross-horizon line plots for one split tag."""
    summary_df.to_csv(
    ...
    for metric in ['RMSE', 'MAPE']:
        ...
        ax.set_title(f'{interval} - {metric} Across Horizons ({tag.title()})',
                     fontsize=14, fontweight='bold')
    ...
    for (model, opt), grp in summary_df.groupby(['Model', 'Optimization']):
        ax.plot(grp['Horizon'], grp['R2_Price'], marker='o',
                linestyle='--' if opt == 'BASE' else '-',
                label=f'{model} ({opt})', linewidth=1.5)
    ax.set_title(f'{interval} - R² (Price Space) Across Horizons ({tag.title()})',
                 fontsize=14, fontweight='bold')
...
def generate_horizon_summary(interval: str,
                             ...
                             output_dir: str):
    """Generate cross-horizon comparison plots and summary CSVs for test and validation."""
...
def run_interval(interval: str, output_dir: str, csa_config: Dict, bayes_config: Dict = None):
    ...
    all_val_results  = {}
    for h in HORIZONS:
        test_df, val_df = run_single_horizon(interval, h, merged_df, output_dir,
                                             csa_config, bayes_config)
...
def main():
    parser = argparse.ArgumentParser(
        description='Multi-Horizon CPO Price Forecasting with CSA and Bayesian Optimization')
    ...
    parser.add_argument('--bayes-calls', type=int, default=50,
                        help='Total Bayesian optimization evaluations (default: 50)')
    parser.add_argument('--bayes-init', type=int, default=10,
                        help='Random initial points before GP guidance (default: 10)')
