"""
Multi-Horizon CPO Price Forecasting with CSA and Bayesian Optimization (CPO + Sentiment)
=========================================================================================
...
Usage:
    ...
    python horizon_forecast_cpo_sentiment.py --interval daily --optimizer bayesian
    python horizon_forecast_cpo_sentiment.py --interval daily --optimizer both
"""
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    ax.set_title(f'{interval} Forecast (CPO + Sentiment, Testing) - Horizon {horizon}', fontsize=14, fontweight='bold')
    ...
    if len(data['val_dates']):
        ...
        for name, preds in val_preds.items():
            ...
            ax.plot(data['val_dates'], data['close_val'] * np.exp(safe),
                    label=name.replace('_', ' ').title(), color=colors.get(name, '#999'),
                    linewidth=1.1, linestyle=ls, alpha=0.8)
        ax.set_title(f'{interval} Forecast (CPO + Sentiment, Validation 2026) - Horizon {horizon}',
    ...
    fig.suptitle(f'{interval} Horizon {horizon} (CPO + Sentiment, Testing) - {opt_names}', fontsize=14, fontweight='bold')
    fig.tight_layout()
...
def _horizon_summary_plots(summary_df: pd.DataFrame, interval: str,
    ...
    for metric in ['RMSE', 'MAPE']:
        ...
        ax.set_title(f'{interval} (CPO + Sentiment) - {metric} Across Horizons ({tag.title()})',
                     fontsize=14, fontweight='bold')
    ...
    for (model, opt), grp in summary_df.groupby(['Model', 'Optimization']):
        ax.plot(grp['Horizon'], grp['R2_Price'], marker='o',
                linestyle='--' if opt == 'BASE' else '-',
                label=f'{model} ({opt})', linewidth=1.5)
    ax.set_title(f'{interval} (CPO + Sentiment) - R² (Price Space) Across Horizons ({tag.title()})',
                 fontsize=14, fontweight='bold')
