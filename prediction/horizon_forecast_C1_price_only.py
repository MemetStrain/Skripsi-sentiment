...
def load_and_merge_data(interval: str) -> pd.DataFrame:
    """Load CPO data only, dropping raw same-day OHLCV columns."""
...
def _save_split_outputs(split_name: str, dates, y_true, close_anchor,
    ...
    """Save per-split predictions and results (matches old C-file format)."""
...
def _save_cv_fold_predictions(cv_pred_rows: List[Dict], horizon_dir: str,
    ...
    """Save per-fold CV predictions (one row per (fold, date, model))."""
    if not cv_pred_rows:
        return
...
def _save_dataset(split: str, dates, X_raw, y, close, feature_cols: List[str],
    ...
    """Save raw (unscaled) feature matrix + target for a given split."""
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    print(f"\n{'='*60}")
    print(f"  {interval} - Horizon {horizon} (C1: price-only)")
    print(f"{'='*60}")
    ...
    for model_type in model_types:
        ...
        # Final BASE model: all pre-test → test
        ...
        if csa_config.get('enabled', True):
            ...
            # CV with CSA params for fair CV comparison
            cv_csa_metrics_list = []
            for fold in data['cv_splits']:
    ...
    if len(data['dates_test']):
        ...
        ax.set_title(f'{interval} Forecast (Test ≥ {VAL_CUTOFF.date()}) - Horizon {horizon} (C1: price-only)',
