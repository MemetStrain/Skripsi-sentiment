"""
Multi-Horizon CPO Price Forecasting with CSA Optimization (CPO Only)
====================================================================
...
Usage:
    ...
    python horizon_forecast_cpo_only.py --interval weekly
    python horizon_forecast_cpo_only.py --interval monthly
    python horizon_forecast_cpo_only.py --interval all
"""
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    ax.set_title(f'{interval} Forecast (CPO Only) - Horizon {horizon}', fontsize=14, fontweight='bold')
    ax.set_xlabel('Date')
    ...
    fig.suptitle(f'{interval} Horizon {horizon} (CPO Only) - Base vs CSA', fontsize=14, fontweight='bold')
    fig.tight_layout()
...
def generate_horizon_summary(interval: str, all_horizon_results: Dict[int, pd.DataFrame],
    ...
    for metric in ['RMSE', 'MAPE']:
        ...
        ax.set_title(f'{interval} (CPO Only) - {metric} Across Horizons', fontsize=14, fontweight='bold')
    ...
    ax.set_title(f'{interval} (CPO Only) - R² (Price Space) Across Horizons', fontsize=14, fontweight='bold')
    ax.set_xlabel('Forecast Horizon')
