"""
Multi-Horizon CPO Price Forecasting with CSA Optimization (CPO + HMM)
=====================================================================

Uses CPO price variables (cpo_variables_{interval}.csv) and
Markov state HMM (hmm_states_results_{interval}.csv).
No sentiment data.
...
Usage:
    python horizon_forecast_cpo_hmm.py --interval daily
    python horizon_forecast_cpo_hmm.py --interval weekly
    python horizon_forecast_cpo_hmm.py --interval monthly
    python horizon_forecast_cpo_hmm.py --interval all
"""
...
# Interval configurations (CPO + HMM)
INTERVAL_CONFIGS = {
    'Daily': {
...
# =============================================================================
# Data Loading (CPO + HMM)
# =============================================================================

def load_and_merge_data(interval: str) -> pd.DataFrame:
    """Load CPO and HMM data, merge into a single DataFrame."""
    ...
    if interval == 'Monthly':
        ...
        merged = cpo.merge(hmm.drop(columns=['Date']), on='_ym', how='inner', suffixes=('', '_hmm'))
        ...
        merged = cpo.merge(hmm.drop(columns=['Date']), on='_yw', how='inner', suffixes=('', '_hmm'))
...
def engineer_features_for_horizon(df: pd.DataFrame, interval: str, horizon: int
    ...
    """
    Build features for a specific forecast horizon using CPO + HMM data.
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    print(f"\n{'='*60}")
    print(f"  {interval} - Horizon {horizon} (CPO + HMM)")
    print(f"{'='*60}")
    ...
    params_data = {
        ...
        'data_sources': ['cpo_variables', 'hmm_states_results'],
    ...
    ax.set_title(f'{interval} Forecast (CPO + HMM) - Horizon {horizon}', fontsize=14, fontweight='bold')
    ax.set_xlabel('Date')
    ...
    fig.suptitle(f'{interval} Horizon {horizon} (CPO + HMM) - Base vs CSA', fontsize=14, fontweight='bold')
    fig.tight_layout()
...
def generate_horizon_summary(interval: str, all_horizon_results: Dict[int, pd.DataFrame],
    ...
    for metric in ['RMSE', 'MAPE']:
        ...
        ax.set_title(f'{interval} (CPO + HMM) - {metric} Across Horizons', fontsize=14, fontweight='bold')
    ...
    ax.set_title(f'{interval} (CPO + HMM) - R² (Price Space) Across Horizons', fontsize=14, fontweight='bold')
    ax.set_xlabel('Forecast Horizon')
...
def run_interval(interval: str, output_dir: str, csa_config: Dict):
    ...
    print(f"\n{'#'*70}")
    print(f"  MULTI-HORIZON FORECAST (CPO + HMM) - {interval.upper()}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Multi-Horizon CPO Price Forecasting - CPO + HMM')
    ...
    output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'output_horizons_cpo_hmm')
