"""
Multi-Horizon CPO Price Forecasting — C2 (Price + HMM)
======================================================

Ablation configuration C2: lagged price + HMM market state (no sentiment).
...
def load_and_merge_data(interval: str) -> pd.DataFrame:
    """Load CPO + HMM data, dropping raw same-day OHLCV columns."""
...
def engineer_features_for_horizon(df: pd.DataFrame, horizon: int
    ...
    """Build features using CPO + HMM data (no look-ahead)."""
    df = df.copy()
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    print(f"\n{'='*60}")
    print(f"  {interval} - Horizon {horizon} (C2: price + HMM)")
    print(f"{'='*60}")
    ...
    if len(data['dates_test']):
        ...
        ax.set_title(f'{interval} Forecast (Testing 2026+) - Horizon {horizon} (C2: price + HMM)',
    ...
    if test_results:
        ...
        fig.suptitle(f'{interval} Horizon {horizon} (Testing, C2) - BASE vs CSA',
                     fontsize=14, fontweight='bold')
        fig.tight_layout()
...
def _horizon_summary_plots(summary_df: pd.DataFrame, interval: str,
    ...
    for metric in ['RMSE', 'MAPE']:
        ...
        ax.set_title(f'{interval} - {metric} Across Horizons (C2, {tag.title()})',
                     fontsize=14, fontweight='bold')
    ...
    for (model, opt), grp in summary_df.groupby(['Model', 'Optimization']):
        ax.plot(grp['Horizon'], grp['R2_Price'], marker='o',
                linestyle='--' if opt == 'BASE' else '-',
                label=f'{model} ({opt})', linewidth=1.5)
    ax.set_title(f'{interval} - R² (Price Space) Across Horizons (C2, {tag.title()})',
                 fontsize=14, fontweight='bold')
...
def run_interval(interval: str, output_dir: str, csa_config: Dict,
    ...
    print(f"\n{'#'*70}")
    print(f"  MULTI-HORIZON FORECAST - {interval.upper()}  (C2: price + HMM)")
