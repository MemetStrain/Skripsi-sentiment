...
# emits (see prediction/master_features.py). C2 = lagged CPO price/technicals
# + HMM regime features; no sentiment.
ABLATION: str = 'C2_cpo_hmm'
...
def run_single_horizon(interval: str, horizon: int, merged_df: pd.DataFrame,
    ...
    if len(data['dates_test']):
        ...
        ax.set_title(f'{interval} Forecast (Test ≥ {VAL_CUTOFF.date()}) - Horizon {horizon} (C2: price + HMM)',
...
def _horizon_summary_plots(summary_df: pd.DataFrame, interval: str,
    ...
    for (model, opt), grp in summary_df.groupby(['Model', 'Optimization']):
        ax.plot(grp['Horizon'], grp['Directional_Accuracy'], marker='o',
                linestyle='--' if opt == 'BASE' else '-',
                label=f'{model} ({opt})', linewidth=1.5)
    ax.set_title(f'{interval} - Directional Accuracy Across Horizons (C2, {tag.title()})',
                 fontsize=14, fontweight='bold')
